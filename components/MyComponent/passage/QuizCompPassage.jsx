...
export default function QuizCompPassage({
  jumlahSoal = 3,
  tipeSoal,
  setIsStarted,
}) {
  ...
  const resetQuiz = async () => {
    ...
    if (tipeSoal === "sinonim") {
      ...
      setQuizData(soalPassage(jumlahSoal).ListSoalFinal);
      setStory({
        story: soalPassage(jumlahSoal).story,
        title: soalPassage(jumlahSoal).title,
      });
    }
    setScore(0);
  ...
  if (isSubmitted) {
    return (
      <Card className="w-full max-w-3xl mx-auto">
        ...
        <CardFooter>
          <Button onClick={exitQuiz} className="w-full cursor-pointer">
            Try Again
          </Button>
        </CardFooter>
      </Card>
